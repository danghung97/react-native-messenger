...
type Server struct {
	connectedUsers map[uint] *User
	//Messages []*models.Messages `json: messages`
	addUser chan *User
	removeUser chan *User
	newIncomingMessage chan *models.Messages
...
func NewServer() *Server {
	...
	connectedUsers := make(map[uint]*User)
	addUser := make(chan *User)
	removeUser := make(chan *User)
	newIncomingMessage := make(chan *models.Messages)
...
func (server *Server) ProcessNewIncomingMessage(message *models.Messages) {
...
func (server *Server) sendAll(msg *models.Messages) {
	log.Println("In Sending to all Connected users")
	for id, user := range server.connectedUsers {
		if id == msg.RoomID {
			user.Write(msg)
		}
	}
...
func (server *Server) Listen() {
	...
	http.HandleFunc("/ws", server.HandleChat)
...
func (server *Server) HandleChat(responseWriter http.ResponseWriter, request *http.Request) {
	...
	//fmt.Println("mssss", messageObject)
	...
	user := NewUser(conn, server, messageObject.RoomID)
